...
Environment is allowlisted. Children automatically receive standard pi model credential variables and every `$VAR` reference from `~/.pi/agent/models.json`, including JSONC comments/trailing commas and nested model/header references. This supports nested delegation without copying unrelated application secrets. Tool and other custom credentials require explicit `PI_SUBAGENT_PASSTHROUGH_ENV` — exact names and globs such as `EXA_API_KEY`, `*_API_KEY`, or `*` are supported; `*` passes all env and is an insecure opt-in. Best practice: store keys via `pi /login` into `~/.pi/agent/auth.json` — then children need no env passthrough at all.
