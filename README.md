...
Only a small environment allowlist is copied. Env vars referenced in `~/.pi/agent/models.json` as `$VAR` (e.g. `$OPENROUTER_API_KEY`, `$META_API_KEY`) are auto-discovered and passed to children — model keys work without manual maintenance. For tool keys not in models.json (e.g. `EXA_API_KEY`), set `PI_SUBAGENT_PASSTHROUGH_ENV` — supports exact names and globs: `*_API_KEY`, `OPENAI_*`. Once-and-done: `PI_SUBAGENT_PASSTHROUGH_ENV="*_API_KEY"` or `*_API_KEY,*_TOKEN`. Use `*` to pass all env (insecure explicit opt-in). Prefer storing keys via `pi /login` into `~/.pi/agent/auth.json` when possible — then children need no env passthrough at all.
