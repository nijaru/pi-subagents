...
Environment is allowlisted. Children automatically get API keys needed to work, with no manual list: (1) all `*_API_KEY` / `*_TOKEN` present in parent env are passed, (2) all `$VAR` refs from `~/.pi/agent/models.json` are passed, preferring the specific provider for the model being spawned but including all model refs so nested `glm -> meta` delegation works. For non-API-key custom vars, set `PI_SUBAGENT_PASSTHROUGH_ENV` — supports exact names and globs `OPENAI_*`, `*_TOKEN`, or `*` for all env (insecure opt-in). Best practice: store keys via `pi /login` into `~/.pi/agent/auth.json` — then children need no env passthrough at all and nothing leaks via env.
