...
Nested delegation is bounded by depth 3, a root-wide budget of 32 descendants, a shared limit of four active child slots, and one root deadline. Nested callers temporarily yield their parent slot while waiting for descendants, so recursive delegation does not starve behind a full sibling fan-out. These are guardrails, not a hostile-code sandbox: a Bash-capable child can intentionally launch processes outside the extension's control file or process group. Controls are propagated through an ephemeral mode-0600 control file and environment IDs; state publication is atomic and malformed control state fails closed. Each child has a 30-minute hard timeout, configurable up to two hours with `PI_SUBAGENT_TIMEOUT_MS` and bounded by the root deadline.
...
The package targets the current Pi CLI/API used by its `@earendil-works/pi-*` 0.83.0 development dependencies. It does not add compatibility shims for older Pi versions.
