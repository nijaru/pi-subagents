...
The implementation relies on the current Pi CLI/API (`--mode json`, `--no-session`, `--tools`/`--no-tools`, and `@file`) and is tested against the 0.82.1 package line; it does not claim compatibility with older Pi versions.
