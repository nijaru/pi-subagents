...
Task and system-prompt contents use mode-0600 temporary files. Child environment variables are allowlisted; API keys referenced in `~/.pi/agent/models.json` via `$VAR` (e.g. `$OPENROUTER_API_KEY`, `$META_API_KEY`) are auto-passed to children. For other provider/tool keys, set `PI_SUBAGENT_PASSTHROUGH_ENV` — supports exact names and globs like `*_API_KEY`, `OPENAI_*`. Use `*_API_KEY` once to avoid list maintenance; `*` passes all env (insecure opt-in). The package does not implement persistent sessions, background registries, or managed worktrees; those are future explicit opt-ins.
